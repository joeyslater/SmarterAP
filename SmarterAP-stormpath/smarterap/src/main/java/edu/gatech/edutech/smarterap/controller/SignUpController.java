...
import edu.gatech.edutech.smarterap.validator.SignUpValidator;
    ...
    public String processSubmit(@ModelAttribute("customer") User user, ModelMap model, BindingResult result, SessionStatus status) {
        ...
        try {
            ...
            if (result.hasErrors()) {
                ...
                model.addAttribute("STUDENT_URL", studentGroupURL);
                model.addAttribute("TEACHER_URL", teacherGroupURL);
            ...
            // Saving the account to the Directory where the Smarter application belongs.
